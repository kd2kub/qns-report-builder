...
# 1.4 - 04/05/2026 - Randomized message number
#
##############################################################################################
...
QNSTODAY=$( date '+%b %d' )
OCTENSESSDATE=$( date "+%b %d" )

echo "Today's date is $NOW"
echo "QNS DATE Listed is $QNSTODAY"
echo "OctenssDate $OCTENSESSDATE"
echo "OCTEN MINUTES $2"
echo "INPUT FILE $1"
...
if [[ ${#MSGALLOC[@]} -gt 1 ]]; then
	...
	do
		NUMB="$((1 + $RANDOM % 99999))"
		echo "$NUMB/$value" >> ./working-$NOW/qns-final-$NOW.txt
	done	
